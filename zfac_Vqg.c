...
int main(int argc,char* argv[])
{
   ...
   FILE *fp_gprops; // read gluon propagator
   ...
   char gprop_name[qcd_MAX_STRING_LENGTH];       // name of inverted Fourier source
   ...
   strcpy(gprop_name,qcd_getParam("<gprop_name>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",gprop_name);
   ...
   if(P[0]*P[1]*P[2]*P[3] != 1){
     fprintf(stderr,"Error only one process is allowed\n");
     exit(EXIT_FAILURE);
   }
   //#####################################################################   
   ...
   if(myid==0)
   {
      ...
      if( (fp_gprops=fopen(gprop_name,"r"))==NULL ) j++;
   ...
   for(mu = 0 ; mu < 4 ; mu++)
     for(nu = 0 ; nu < 4 ; nu++)
       for(c1 = 0 ; c1 < 3 ; c1++)
	 for(c2 = 0 ; c2 < 3 ; c2++){
	   fscanf(fp_gprops,"%d %d %d %d %lf %lf",&d_dummy,&d_dummy,&d_dummy,&d_dummy,&(gprop[mu][nu][c1][c2].re), &(gprop[mu][nu][c1][c2].im));
	 }
   ...
   ////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 for(nu=0; nu<4; nu++)
	   for(c1=0; c1<3; c1++)
	     for(c2=0; c2<3; c2++)
	       {
		 fprintf(fp_vfun_vD,"%d \t %d %d %d %d \t %+e %+e\n",i,mu,nu,c1,c2,gprop[mu][nu][c1][c2].re * loops_vD[i], gprop[mu][nu][c1][c2].im * loops_vD[i]);
	       }
