...
#include <mpi.h>
#include <qcd.h>
#include <qcd_modified.h>
#include "my_projectors.h"
#include "lib/src/qcd_gamma_up.h"
...
int main(int argc, char* argv[]){
 ...
 qcd_uint_2 mu,nu,ku,lu,lambda,xita,beta,rho,phita, delta, sigma,zita, kappa; // dirac indices
  ...
  int kk,Tseperation;               // this index takes the gamma matrix fo insertion operator
  ...
  list_projectors[3] = 3;  // -> g5*(g1 + g2 + g3)                                                                                            // store three point function

  int global_id,local_id,group_id;
  ...
  if(argc != 2)
    {
      if(global_id == 0) fprintf(stderr,"No input file specified \n"); // check for input file
      exit(EXIT_FAILURE);
    }
  ...
  MPI_Bcast(&i, 1, MPI_INT, 0, global_comm); // broadcast success or fail to all
  if(i == 1) exit(EXIT_FAILURE); 

  MPI_Bcast(&params_len, 1, MPI_INT, 0, global_comm); // broadcast params_len to all

  if(global_id != 0) params = (char*)malloc(params_len*sizeof(char)); // allocate memory for params for all except root beacause a previous routine do that for root

  MPI_Bcast(params, params_len, MPI_CHAR, 0, global_comm); // Broadcast the params to all processors
  ...
  sscanf(qcd_getParam("<local_par_time>",params,params_len),"%d",&local_par_time);
  if(global_id == 0) printf("Got number of time local blocks = %d\n",local_par_time);

  group_id = global_id % local_par_time ;
  local_id = global_id / local_par_time ;
  MPI_Comm_split(global_comm,group_id,local_id,&local_comm);
  
  if(qcd_initGeometry(&geo, L, P, theta, local_id, numprocs/local_par_time)) exit(EXIT_FAILURE);  // start the geometry each processor takes a segment of lattice
  myid = local_id;
  
  if(global_id == 0) printf( " Local Lattice : %i x %i x %i x %i \n", geo.lL[0], geo.lL[1], geo.lL[2], geo.lL[3]); // print the local lattice

  strcpy(output_name,qcd_getParam("<output_name>",params,params_len)); //get  output filename                                                                
  if(global_id==0) printf("Got output file name: %s\n",output_name);

  strcpy(uprop_name,qcd_getParam("<propagator_u>",params,params_len)); //get  u propagator file list
  if(global_id==0) printf("Got propagator file name: %s\n",uprop_name);
  ...
  if(global_id==0) printf("Got source coords: %d %d %d %d\n",x_src[0],x_src[1],x_src[2],x_src[3]);

  sscanf(qcd_getParam("<current_pos_t>",params,params_len),"%d",&t_noise);                                              // read the time position of insertion
  if(global_id==0) printf("Got noise vector time-slice dilution: %d \n",t_noise);

   sscanf(qcd_getParam("<Tseperation>",params,params_len),"%d",&Tseperation);                                         // index to choose the projector                                   
  if(global_id==0) printf("Got time lattice points between source and sink : %d \n",Tseperation);

  
  free(params); 

  if( (Tseperation % local_par_time) != 0) {
    if(global_id == 0 ) fprintf(stderr,"problem seperate time to processors \n");
    exit(EXIT_FAILURE);
  }
  ...
  MPI_Allreduce(&j, &k, 1, MPI_INT, MPI_SUM, global_comm);
  if(k>0)
    {
      if(global_id==0) fprintf(stderr,"Error, not enough memory\n");
      exit(EXIT_FAILURE);
    }
  if(qcd_getGaugeField(gauge_name,qcd_GF_LIME,&u))
    {
      if(global_id==0) fprintf(stderr,"Error reading gauge field\n");
      exit(EXIT_FAILURE);
    }
    
   
  if(global_id==0) printf("gauge-field loaded\n");
  
  plaq = qcd_calculatePlaquette_modified(&u,local_comm);
  if(global_id==0) printf("plaquette = %e\n",plaq);
  
  qcd_communicateGaugePM_modified(&u,local_comm);
  qcd_waitall(&geo);
  u_ptr = &u;
  uAPE_ptr = &uAPE;
  if(global_id == 0 ) printf("start APE smearing\n");
  for(i=0; i<nsmearAPE; i++)
    {
      qcd_apeSmear3d_modified(uAPE_ptr, u_ptr, alphaAPE,local_comm);
  ...
  uAPE = *uAPE_ptr;
  if(global_id == 0) printf("printf finish APE smearing\n");
  plaq = qcd_calculatePlaquette_modified(&uAPE,local_comm);
  if(global_id==0) printf("Smeared plaquette = %e\n",plaq);
  qcd_waitall(&geo);
  ...
  if(k>0) // check for memory allocation flag
    {
      if(global_id==0) printf("not enough memory\n");
      exit(EXIT_FAILURE);
    }
  if(global_id==0) printf("memory for propagators and tsm vectors allocated\n");
  ...
  if(global_id==0) printf("propagators transformed to basis with theta-periodic boundary conditions\n");
  ...
  for(mu=0;mu<4;mu++)
    for(c1=0;c1<3;c1++)
      {
	if(global_id == 0)printf("smearing mu = %d , c1 = %d \n",mu,c1);
			qcd_copyVectorPropagator(&vec,&uprop,mu,c1);             // copy each of 12 vectors for propagator to do smearing
		
	for(i=0; i<nsmear; i++)
	  {
	    
	    if(qcd_gaussIteration3d_modified(&vec,&uAPE,alpha,t_noise,local_comm))   // smearing for up propagator
	      {
	...
	for(i=0; i<nsmear; i++)
	  {
	    ...
	    if(qcd_gaussIteration3d_modified(&vec,&uAPE,alpha,t_noise,local_comm))        // smearing for down propagator
	      {
		fprintf(stderr,"process %i: Error while smearing!\n",global_id);
		exit(EXIT_FAILURE);
	      }
  ...
  if(global_id==0) printf("propagators smeared\n");
...
///////////////////////////////////////////////////////////////////
  ...
  MPI_Bcast(&k,1,MPI_INT, 0, global_comm);
  if(k==1) exit(EXIT_FAILURE); // check fo success
  //////////////////////////////

  // load the momenta list///////////////////////////////
  if(global_id==0) fscanf(fp_momlist,"%d\n",&num_momenta);
  
  MPI_Bcast(&num_momenta,1,MPI_INT, 0, MPI_COMM_WORLD);
  if(global_id==0) printf("will read %d momenta combinations\n",num_momenta);
  ...
  if(global_id==0) printf("momenta list read and broadcasted\n");
  ////////////////////// succesfuly pass the momenta to all cpu
  
  //allocate memory for all the W terms ///////////////////////////////////////////////
  for(i=0; i < Nmu ; i++)
    for(j=0; j < Nic ; j++){
      W1[i][j] = (qcd_complex_16*)allocate(geo.lV3 * sizeof(qcd_complex_16));
      W7[i][j] = (qcd_complex_16*)allocate(geo.lV3 * sizeof(qcd_complex_16));
      W1p[i][j] = (qcd_complex_16*)allocate(num_momenta * sizeof(qcd_complex_16));
      W7p[i][j] =(qcd_complex_16*)allocate(num_momenta *sizeof(qcd_complex_16));
      W1p_r[i][j] =(qcd_complex_16*)allocate(num_momenta *sizeof(qcd_complex_16));
      W7p_r[i][j] =(qcd_complex_16*)allocate(num_momenta *sizeof(qcd_complex_16));
      for(int gamma_index = 0; gamma_index < Noperators ; gamma_index++){
	W2[gamma_index][i][j] = (qcd_complex_16*)allocate(geo.lV3 * sizeof(qcd_complex_16));
	W2p[gamma_index][i][j] =(qcd_complex_16*)allocate(num_momenta *sizeof(qcd_complex_16));
	W2p_r[gamma_index][i][j] =(qcd_complex_16*)allocate(num_momenta *sizeof(qcd_complex_16));
      }
    }
  if(global_id == 0)printf("finish allocation 1\n");
  ...
  for( i=0; i < Nmu ; i++)
	...
	for(l=0; l<Nic ; l++){
	  ...
	  W5[i][j][k][l] = (qcd_complex_16*)allocate(geo.lV3 * sizeof(qcd_complex_16));
	  W3p[i][j][k][l] = (qcd_complex_16*)allocate(num_momenta * sizeof(qcd_complex_16));
	  W5p[i][j][k][l] =(qcd_complex_16*)allocate(num_momenta * sizeof(qcd_complex_16));
	  W3p_r[i][j][k][l] = (qcd_complex_16*)allocate(num_momenta * sizeof(qcd_complex_16));
	  W5p_r[i][j][k][l] =(qcd_complex_16*)allocate(num_momenta * sizeof(qcd_complex_16));
	}
  if(global_id == 0)printf("finish allocation 2\n");

  for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
    threep_proj[gamma_index] = (qcd_complex_16*)allocate(num_momenta*(Tseperation/local_par_time)*sizeof(qcd_complex_16));
    for( i = 0 ; i < Nmu ; i++)
      for(j = 0 ; j <Nmu ; j++)
	{
	  W12[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*sizeof(qcd_complex_16));
	  W34[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*sizeof(qcd_complex_16));
	  W56[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*sizeof(qcd_complex_16));
	  W78[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*sizeof(qcd_complex_16));
	  threep[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*(Tseperation/local_par_time)*sizeof(qcd_complex_16));
	}
  }
  if(global_id == 0)printf("finish allocation 3\n");

  for(int gamma_index =0 ;gamma_index < Noperators ; gamma_index++){
    for(kk=0; kk < num_momenta*(Tseperation/local_par_time) ; kk++){
  ...
  if(fp_phi_name == NULL){
    if(global_id == 0)fprintf(stderr,"Error opening list with phi vectors\n");
    exit(EXIT_FAILURE);
  }
  if(fp_xi_1_name == NULL){
    if(global_id == 0)fprintf(stderr,"Error opening list with xi_1 vectors\n");
    exit(EXIT_FAILURE);
  }
...
fclose(fp_phi_name);
  ...
  for(int r = 0 ; r < num_phi ; r++){        // a summation over the ensemble of noise vectors
    ...
    for(i=0; i<nsmear; i++)
      {
    	if(qcd_gaussIteration3d_modified(&xi_1,&uAPE,alpha,t_noise,local_comm))
    	  {
    	    fprintf(stderr,"process %i: Error while smearing!\n",global_id);
    	    exit(EXIT_FAILURE);
    	  }
    ...
    */	
	...
	for(k=0; k<Nmu ; k++)
	  for(l=0; l<Nic ; l++)
	    ...
	    ///////////////////////////////////////////////////////////////////////
    if(global_id == 0)printf("start W1 W3 W5 W7 for r = %d \n",r);
	...
	for(lz = 0 ; lz < geo.lL[3] ; lz++){
	  ...
	  for(cc1 = 0 ; cc1 < 6 ; cc1++){
		...
		for(sigma =0 ; sigma < 4 ; sigma++)
		  W3[delta][sigma][lambda][ff][v3] = qcd_CADD(W3[delta][sigma][lambda][ff][v3],qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( cg5cg5g5_val[ctr2] ,
						     qcd_SGN_EPS[cc1]*qcd_SGN_EPS[cc2]), xi_1.D[v][kappa][cc]),uprop.D[v][beta][sigma][aa][hh]),
    ...
    if(global_id ==0) printf("finish W1 W3 W5 W7 for r = %d \n",r);
    
 

    for( it = 0 ; it < Tseperation/local_par_time ; it++)                                         
      {
	if(global_id == 0)printf("start W2 r = %d , t = %d\n",r,it);
	t = (( group_id*(Tseperation/local_par_time) +it+x_src[0])%geo.L[0]);
	for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
	  for(i=0; i < Nmu ; i++)
	    for(j=0; j < Nic ; j++)
	      for(v=0; v<geo.lV3; v++)
	...
	if(global_id == 0)printf("start fourier for r = %d, t = %d\n",r,it);
	...
	for( j=0 ; j< num_momenta ; j++){ // begin fourier
	  ...
	  for( hh = 0 ; hh < Nic ; hh++) // reduce the values to root
	    for(delta = 0 ;delta <Nmu ; delta++){
	      MPI_Reduce(&(W1p[delta][hh][j].re), &(W1p_r[delta][hh][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, local_comm);
	      for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++)
		MPI_Reduce(&(W2p[gamma_index][delta][hh][j].re), &(W2p_r[gamma_index][delta][hh][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, local_comm);
	      MPI_Reduce(&(W7p[delta][hh][j].re), &(W7p_r[delta][hh][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, local_comm);
	      for( mu = 0 ; mu < Nmu ; mu++)
		for(nu = 0 ; nu < Nmu ; nu++){
		  MPI_Reduce(&(W3p[delta][mu][nu][hh][j].re), &(W3p_r[delta][mu][nu][hh][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, local_comm);
		  MPI_Reduce(&(W5p[delta][mu][nu][hh][j].re), &(W5p_r[delta][mu][nu][hh][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, local_comm);
		}
	    }
	...
	for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
	  for( int q = 0 ; q < num_momenta; q++)
		...
		for( hh = 0 ; hh < Nic ; hh++){
		  W12[gamma_index][delta][sigma][q] = qcd_CADD(W12[gamma_index][delta][sigma][q],qcd_CMUL(W1p_r[delta][hh][0] , W2p_r[gamma_index][sigma][hh][q]));
		  W78[gamma_index][delta][sigma][q] = qcd_CADD(W78[gamma_index][delta][sigma][q],qcd_CMUL(W7p_r[delta][hh][0] , W2p_r[gamma_index][sigma][hh][q]));
		  
		  for(int lambda = 0 ; lambda < Nmu ; lambda++){
		     W34[gamma_index][delta][sigma][q] = qcd_CADD(W34[gamma_index][delta][sigma][q],qcd_CMUL(W3p_r[delta][sigma][lambda][hh][0],W2p_r[gamma_index][lambda][hh][q]));
		     W56[gamma_index][delta][sigma][q] = qcd_CADD(W56[gamma_index][delta][sigma][q],qcd_CMUL(W5p_r[delta][sigma][lambda][hh][0],W2p_r[gamma_index][lambda][hh][q]));
		  }
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
	    for( delta = 0 ; delta < Nmu ; delta++)
	      for( sigma = 0 ; sigma < Nmu ; sigma++){
		threep[gamma_index][delta][sigma][q*(Tseperation/local_par_time)+it] = qcd_CADD(threep[gamma_index][delta][sigma][q*(Tseperation/local_par_time)+it],qcd_CADD(qcd_CSUB(W12[gamma_index][delta][sigma][q],
               W34[gamma_index][delta][sigma][q]),qcd_CSUB(W56[gamma_index][delta][sigma][q], W78[gamma_index][delta][sigma][q])));
	      }
	}
	...
	if(local_id == 0){
	  for(int proj_index = 0 ; proj_index < 4 ; proj_index++)
	    for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
	      for( int q = 0 ; q < num_momenta; q++)
		for(int it = 0 ; it < Tseperation/local_par_time ; it++)
		  {
		    threep_proj[gamma_index][q*(Tseperation/local_par_time)+it] = (qcd_complex_16) {0,0};  		  
		    for(int delta = 0 ; delta < Nmu ; delta++)
		      for(int sigma = 0 ; sigma < Nmu ; sigma++){
			threep_proj[gamma_index][q*(Tseperation/local_par_time)+it] = qcd_CADD(threep_proj[gamma_index]
			[q*(Tseperation/local_par_time)+it],qcd_CMUL(PROJECTOR[list_projectors[proj_index]][delta][sigma],
								     threep[gamma_index][sigma][delta][q*(Tseperation/local_par_time)+it]));   
		      }
		      fprintf(fp_threep[proj_index],"%d %d %d %d %+e %+e\n",r,gamma_index, group_id*(Tseperation/local_par_time) +it,q,(threep_proj[gamma_index][q*(Tseperation/local_par_time)+it].re)/(r+1),(threep_proj[gamma_index][q*(Tseperation/local_par_time)+it].im)/(r+1));
		  }
	    }  
	}
  ...
  if(global_id == 0)printf("finish program \n");
