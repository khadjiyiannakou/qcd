...
int tpf_sigma_star_minus_J32(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, qcd_propagator *seq_dprop,
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
		  ...
		  D_part_sigmaSM[mu][nu][itmom] = qcd_CSUB( Term3_D[mu][nu] , Term6_D[mu][nu] ) ;
		  ...
		  S_part_sigmaSM[mu][nu][itmom] = qcd_CSUB( Term3_S[mu][nu] , Term6_S[mu][nu] ) ;
