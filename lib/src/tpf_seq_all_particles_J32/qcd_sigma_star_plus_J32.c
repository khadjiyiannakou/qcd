...
int tpf_sigma_star_plus_J32(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, qcd_propagator *seq_dprop,
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[i*3+j][v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
		  ...
		  U_part_sigmaSP[mu][nu][itmom] = qcd_CSUB( Term3_U[mu][nu] , Term6_U[mu][nu] ) ;
		  ...
		  S_part_sigmaSP[mu][nu][itmom] = qcd_CSUB( Term3_S[mu][nu] , Term6_S[mu][nu] ) ;
