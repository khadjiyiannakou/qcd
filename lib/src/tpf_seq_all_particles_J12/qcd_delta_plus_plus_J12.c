...
int tpf_delta_plus_plus_J12(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, 
			    qcd_propagator *seq_dprop,qcd_propagator *seq_sprop, qcd_geometry *geo, qcd_int_2 cur_time, qcd_int_2 number_tsinks, qcd_complex_16 *U_part_deltaPP[4][4], qcd_complex_16 *D_part_deltaPP[4][4], qcd_complex_16 *S_part_deltaPP[4][4],qcd_int_4(*mom)[3], int num_momenta, qcd_uint_4 *x_src, qcd_int_2 after_tcur){
    ...
    for(int it = 0; it < number_tsinks ; it++){
	  ...
	  for(int inz =0 ; inz < nz_counter[i*3+j] ; inz++){
	    ...
	    for(int cc1=0;cc1<6;cc1++)
			...
			////////// Upart ////////////////
			block_Upart[gamma][gamma1][i*3+j][v3] = qcd_CSUB( block_Upart[gamma][gamma1][i*3+j][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
		  ...
		  U_part_deltaPP[mu][nu][itmom] = qcd_CADD( Term3_U[mu][nu] , Term6_U[mu][nu] ) ;
