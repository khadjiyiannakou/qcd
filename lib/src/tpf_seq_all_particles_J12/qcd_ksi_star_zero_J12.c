	  ...
	  for(int inz =0 ; inz < nz_counter[i*3+j] ; inz++){
	    ...
	    for(int cc1=0;cc1<6;cc1++)
			...
			block_Upart[gamma][gamma1][i*3+j][v3] = qcd_CSUB( block_Upart[gamma][gamma1][i*3+j][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 ...
			                                 ,sprop->D[v][gamma][alpha1][c][a1] ), seq_uprop->D[v][beta][beta1][b][b1] ));
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
		  ...
		  U_part_ksiSZ[mu][nu][itmom] = qcd_CADD( Term3_U[mu][nu] , Term6_U[mu][nu] ) ;
		  ...
		  S_part_ksiSZ[mu][nu][itmom] = qcd_CADD( Term3_S[mu][nu] , Term6_S[mu][nu] ) ;
