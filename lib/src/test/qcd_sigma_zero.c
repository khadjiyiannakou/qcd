...
int tpf_sigma_zero_test(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, qcd_propagator *seq_dprop,
     ...
     sprintf(filename2,"%s_%d.dat",filename,geo->myid);
     
     block_file = fopen(filename2,"w");

     //     if(geo->myid == 0)test_file = fopen("kale.dat","w");
     //     if(geo->myid == 0)test_file = fopen("reduce.dat","w");
   ...
   for(int alpha=0 ; alpha < 4; alpha++)
    ...
    for(int it = 0; it < number_tsinks ; it++){
			...
			*/
			 ...
			 for(int gamma1 = 0 ; gamma1 < 4 ; gamma1++)
			   for(int lx=0; lx < geo->lL[1]; lx++)
				 ...
				 {
				   v3 = qcd_LEXIC0(lx,ly,lz,geo->lL);
				   fprintf(block_file,"%d %d %d %+e %+e\n",gamma,gamma1,v3,block_Dpart[gamma][gamma1][v3].re,block_Dpart[gamma][gamma1][v3].im);
				 }
	  ...
	  for(int imom = 0; imom < num_momenta; imom++){
	    ...
	    if(geo->myid == 0){
	      for(int gamma = 0 ; gamma < 4 ; gamma++)
		fprintf(test_file,"%e %e  %e %e %e %e %e %e\n",tmp_corr[1][gamma][0].re,tmp_corr[1][gamma][0].im,tmp_corr[1][gamma][1].re,tmp_corr[1][gamma][1].im,tmp_corr[1][gamma][2].re,tmp_corr[1][gamma][2].im,tmp_corr[1][gamma][3].re,tmp_corr[1][gamma][3].im);
	    }
	    */
	      itmom = it * num_momenta + imom ;
            for(int gamma =0 ; gamma < 4 ; gamma++)
	      for(int gamma1 =0 ;gamma1 < 4 ; gamma1++){
		MPI_Allreduce(&(tmp_corr[0][gamma][gamma1].re), &(U_part_sigmaZ[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM,MPI_COMM_WORLD );
		MPI_Allreduce(&(tmp_corr[1][gamma][gamma1].re), &(D_part_sigmaZ[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM,MPI_COMM_WORLD );
		MPI_Allreduce(&(tmp_corr[2][gamma][gamma1].re), &(S_part_sigmaZ[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM,MPI_COMM_WORLD );
		...
		for(int gamma1 =0 ; gamma1 < 4 ; gamma1++){
		  ...
		  U_part_sigmaZ[gamma][gamma1][itmom] = qcd_CSCALE(U_part_sigmaZ[gamma][gamma1][itmom] , 0.5);
		  D_part_sigmaZ[gamma][gamma1][itmom] = qcd_CSCALE(D_part_sigmaZ[gamma][gamma1][itmom] , 0.5);
		  S_part_sigmaZ[gamma][gamma1][itmom] = qcd_CSCALE(S_part_sigmaZ[gamma][gamma1][itmom] , 0.5);
