/* KYRIAKOS HADJIYIANNAKOU 
 Neutron 3pf using sequential throw the current */
...
int threep_neutron(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, qcd_propagator *seq_dprop,
		  qcd_propagator *seq_sprop, qcd_geometry *geo, qcd_int_2 cur_time, qcd_int_2 number_tsinks, qcd_complex_16 *U_part_neutron[5], 
		  qcd_complex_16 *D_part_neutron[5], qcd_complex_16 *S_part_neutron[5],qcd_int_4(*mom)[3], int num_momenta, qcd_uint_4 *x_src, qcd_int_2 after_tcur, int list_projectors[5]){
	  ...
	  for(int inz =0 ; inz < nz_counter ; inz++){
	    ...
	    for(int cc1=0;cc1<6;cc1++)
			...
			block_Dpart[gamma][gamma1][v3] = qcd_CSUB( block_Dpart[gamma][gamma1][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 cg5cg5b_val[inz], qcd_SGN_EPS[cc1]*qcd_SGN_EPS[cc2]),uprop->D[v][beta][beta1][b][b1] )
			...
			block_Dpart[gamma][gamma1][v3] = qcd_CADD( block_Dpart[gamma][gamma1][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 cg5cg5b_val[inz], qcd_SGN_EPS[cc1]*qcd_SGN_EPS[cc2]),uprop->D[v][beta][beta1][b][b1] )
			                                 ,dprop->D[v][alpha][alpha1][a][a1] ), seq_dprop->D[v][gamma][gamma1][c][c1] ));
			
			block_Dpart[gamma][gamma1][v3] = qcd_CSUB( block_Dpart[gamma][gamma1][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 cg5cg5b_val[inz], qcd_SGN_EPS[cc1]*qcd_SGN_EPS[cc2]),uprop->D[v][beta][beta1][b][b1] )
			                                 ,dprop->D[v][alpha][gamma1][a][c1] ), seq_dprop->D[v][gamma][alpha1][c][a1] ));
			////////////////////////////////
			/////////Upart//////////////////
			...
			block_Upart[gamma][gamma1][v3] = qcd_CSUB( block_Upart[gamma][gamma1][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 ...
			                                 ,dprop->D[v][gamma][alpha1][c][a1] ), seq_uprop->D[v][beta][beta1][b][b1] ));
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
	      ...
	      U_part_neutron[i][itmom] = sum[0];
	      D_part_neutron[i][itmom] = sum[1];
	      S_part_neutron[i][itmom] = sum[2];
	    }
