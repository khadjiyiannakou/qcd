...
int threep_octet_uds(qcd_propagator *uprop, qcd_propagator *dprop, qcd_propagator *sprop, qcd_propagator *seq_uprop, qcd_propagator *seq_dprop,
		  ...
		  qcd_complex_16 *D_part[5], qcd_complex_16 *S_part[5],qcd_int_4(*mom)[3], int num_momenta, qcd_uint_4 *x_src, qcd_int_2 after_tcur, int list_projectors[5]){
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int nu = 0 ; nu < 4 ; nu++){
	      ...
	      U_part[i][itmom] = sum[0];
	      D_part[i][itmom] = sum[1];
	      S_part[i][itmom] = sum[2];
	    }
