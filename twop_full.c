...
int main(int argc, char* argv[]){
  ...
  qcd_int_4 (*mom)[3];                         // momenta-list
   ...
   qcd_complex_16 corr_temp[MAX_PARTICLES_2][16], corr[MAX_PARTICLES_2][16];
   ...
   char *particles[MAX_PARTICLES_2] = {"PROTON", "NEUTRON", "LAMBDA", "SIGMA_PLUS", "SIGMA_ZERO", "SIGMA_MINUS", 
   "KSI_ZERO", "KSI_MINUS", "DELTA_PLUS_PLUS", "DELTA_PLUS", "DELTA_ZERO", "DELTA_MINUS", "SIGMA_STAR_PLUS", "SIGMA_STAR_ZERO",
   "SIGMA_STAR_MINUS", "KSI_STAR_ZERO", "KSI_STAR_MINUS", "OMEGA", "LAMBDA_C", "SIGMA_PLUS_PLUS_C", "SIGMA_PLUS_C", "SIGMA_ZERO_C", 
   "KSI_PLUS_PLUS_C_C", "KSI_PLUS_C_C", "SIGMA_STAR_PLUS_PLUS_C", "SIGMA_STAR_PLUS_C", "SIGMA_STAR_ZERO_C", "KSI_STAR_PLUS_PLUS_C_C", 
   "KSI_STAR_PLUS_C_C", "OMEGA_PLUS_PLUS_C_C_C", "OMEGA_ZERO_C", "KSI_PRIME_ZERO_C", "KSI_ZERO_C", "KSI_PRIME_PLUS_C", "KSI_PLUS_C", 
   "OMEGA_PLUS_C_C", "OMEGA_STAR_ZERO_C", "KSI_STAR_ZERO_C", "KSI_STAR_PLUS_C", "OMEGA_STAR_PLUS_C_C"};
	 ...
	 //////////////////////////////// start fourier /////////////////////////////////////////////
	  ...
	  for(int imom = 0; imom < num_momenta; imom++){
	   ...
	   for(int mu = 0 ; mu < 4 ; mu++)
	     for(int nu = 0 ; nu < 4 ; nu++)
		 for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
		   corr_temp[iparticle][mu*4+nu] = (qcd_complex_16) {0,0} ;
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
	    ...
	    for(int mu = 0 ; mu < 4 ; mu++)
	      for(int nu =0 ; nu < 4 ; nu++)
		  for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
		    MPI_Reduce(&(corr_temp[iparticle][mu*4+nu].re),&(corr[iparticle][mu*4+nu].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
		...
		for(int mu = 0 ; mu < 4 ; mu++)
                  fprintf(fp_twop[iparticle], "%d %d %+e %+e %+e %+e %+e %+e %+e %+e \n", it+after_tcur, imom, corr[iparticle][mu*4+0].re, corr[iparticle][mu*4+0].im
                  ,corr[iparticle][mu*4+1].re, corr[iparticle][mu*4+1].im,corr[iparticle][mu*4+2].re, corr[iparticle][mu*4+2].im,corr[iparticle][mu*4+3].re, corr[iparticle][mu*4+3].im);
