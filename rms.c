...
//////////////////// READ INPUT FILE /////////////////////////////////////////////
   ...
   for(double alpha=alpha_i; alpha <= alpha_f; alpha+=d_alpha)
     {       
       qcd_zeroVector(&vec); 
       if( (lx_src[0]>=0) && (lx_src[0]<geo.lL[0]) && 
	   (lx_src[1]>=0) && (lx_src[1]<geo.lL[1]) && 
	   (lx_src[2]>=0) && (lx_src[2]<geo.lL[2]) && 
	   (lx_src[3]>=0) && (lx_src[3]<geo.lL[3]))
	 vec.D[qcd_LEXIC(lx_src[0],lx_src[1],lx_src[2],lx_src[3],geo.lL)][0][0].re=1.;
	   ...
	   if( (lx_src[0]>=0) && (lx_src[0]<geo.lL[0]) )
		 ...
		 for(int y=0; y<geo.lL[2]; y++)
		   for(int x=0; x<geo.lL[1]; x++)
		     {
		       int gx = geo.lL[1]*geo.Pos[1] + x;
		       int gy = geo.lL[2]*geo.Pos[2] + y;
		       int gz = geo.lL[3]*geo.Pos[3] + z;

		       gx = gx>geo.L[1]/2 ? geo.L[1] - gx : gx;
		       gy = gy>geo.L[2]/2 ? geo.L[2] - gy : gy;
		       gz = gz>geo.L[3]/2 ? geo.L[3] - gz : gz;

		       int lv = qcd_LEXIC(lx_src[0], x, y, z, geo.lL);
		       double loc_norm = 0;
