...
int main(int argc, char* argv[]){
  ...
  qcd_complex_16 *threep[Nmu][Nmu], *threep_proj;                                                                                                 // store three point function
 ...
 //set up MPI                                                                                 
  ...
  sscanf(qcd_getParam("<gamma_index>",params,params_len),"%d",&gamma_index);                                         // index to choose Projector
  ...
  for(zita = 0 ; zita < 4 ; zita++)
	...
	sum = qcd_CADD(sum,qcd_CMUL(qcd_GAMMA[5][zita][rho],qcd_G5GAMMA[gamma_index][rho][phita]));
      ...
      gammag5[zita][phita].re = sum.im ;
      gammag5[zita][phita].im = -sum.re ;  // multiply by -i so i have -i*Gamma for insertion
    }
  ...
  if(myid == 0){
    for( int q = 0 ; q < num_momenta; q++)
      for(int it = x_src[0] ; it < t_noise ; it++)
        {
          ...
          for(int delta = 0 ; delta < Nmu ; delta++)
            for(int sigma = 0 ; sigma < Nmu ; sigma++){
              threep[delta][sigma][q*geo.L[0]+t].re = (threep[delta][sigma][q*geo.L[0]+t].re/num_phi);
              threep[delta][sigma][q*geo.L[0]+t].im = (threep[delta][sigma][q*geo.L[0]+t].im/num_phi);
            }

          for(int delta = 0 ; delta < Nmu ; delta++)
            for(int sigma = 0 ; sigma < Nmu ; sigma++){
	      threep_proj[q*geo.L[0]+t] = qcd_CADD(threep_proj[q*geo.L[0]+t],qcd_CMUL(PROJECTOR[proj_index][delta][sigma],threep[sigma][delta][q*geo.L[0]+t]));
            }

          fprintf(fp_threep,"%d %d %+e %+e\n",t-x_src[0],q,threep_proj[q*geo.L[0]+t].re,threep_proj[q*geo.L[0]+t].im);
