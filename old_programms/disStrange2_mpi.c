...
//   if(myid==0) printf("Got solution-list file name: %s\n",sollist_name);
   ...
   for(mu=0;mu<Nmu;mu++)
   for(nu=0;nu<Nmu;nu++){
    C = qcd_G5GAMMAG5[gamma_index][mu][nu];
    if(qcd_NORM(C) > 1e-3){
    ...
    for(int it=0;it<Phi.geo->lL[0];it++){
     ...
     for(int ctr2=0;ctr2<ctr;ctr2++){     
      for(int ic=0;ic<Nic;ic++){
	   ...
	   mu = gamma_ind[ctr2][0];
           nu = gamma_ind[ctr2][1];
           block[v3] =qcd_CADD(block[v3],qcd_CMUL(gamma_val[ctr2],qcd_CMUL(conju(Phi.D[qcd_LEXIC(it,lx,ly,lz,geo.lL)][mu][ic]),Phi.D[qcd_LEXIC(it,lx,ly,lz,geo.lL)][nu][ic])));
...
MPI_Reduce(&(loop.re),&(loop2[(ns)*Nmom*L[0]+(j)*L[0]+it]),2,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
