...
int main(int argc, char* argv[]){
  ...
  qcd_int_4 gamma_index,kk,proj_index;               // this index takes the gamma matrix fo insertion operator
  ...
  qcd_int_4 ctr, ctr2, ctr11, ctr22, ctr33;           // special spin indices trick for gammas multiplications  
  qcd_int_2 cg5cg5g5_ind[16*16*16][6], gammag5_ind[16*16][4], cg5g5cg5_ind[16*16][4]; // store the non zero indices and gamma values
  qcd_complex_16 cg5cg5g5_val[16*16*16], gammag5_val[16*16], cg5g5cg5_val[16*16];  // %
  qcd_complex_16 gammag5[4][4], g5cg5[4][4];
  ...
  qcd_complex_16 *threep[Nmu][Nmu], *threep_proj,*threep_test[Nmu][Nmu];                                                                                                 // store three point function
 ...
 //set up MPI                                                                                 
  ...
  sscanf(qcd_getParam("<proj_index>",params,params_len),"%d",&proj_index);                                         // index to choose the gamma insertion matrix                                   
  ...
  for(zita = 0 ; zita < 4 ; zita++)
    for(phita = 0 ; phita < 4 ; phita++){
      sum = (qcd_complex_16) {0,0};
      for( rho = 0 ; rho < 4 ; rho++){
	sum = qcd_CADD(sum,qcd_CMUL(qcd_GAMMA[5][zita][rho],qcd_G5GAMMA[gamma_index][rho][phita]));
      }
      gammag5[zita][phita] = sum ;
    }

  ctr11 = 0 ;
  for(zita =0; zita < 4 ; zita++)                               // now for gamma inserion matrix only
    for(phita=0; phita < 4 ; phita++){
      C = gammag5[zita][phita] ;
      if(qcd_NORM(C) > 1e-3)
	{
	  gammag5_val[ctr11].re = C.re;
	  gammag5_val[ctr11].im = C.im;
	  gammag5_ind[ctr11][0] = zita;
	  gammag5_ind[ctr11][1] = phita;
	  ctr11++;
	}
    }
  ...
  for( i = 0 ; i < Nmu ; i++)
    for(j = 0 ; j <Nmu ; j++)
      {
	W12[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	W34[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	W56[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	W78[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	threep[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	threep_test[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
  ...
  for( i = 0 ; i < Nmu ; i++)
	...
	{
	  threep[i][j][kk] = (qcd_complex_16) {0,0}; // initialize to zero the three point function
	  threep_test[i][j][kk] = (qcd_complex_16) {0,0};
	}

  threep_proj = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
  for(kk=0; kk < num_momenta*geo.L[0] ; kk++) threep_proj[kk] = (qcd_complex_16) {0,0};
...
fclose(fp_phi_name);
  ...
  for(int r = 0 ; r < num_phi ; r++){        // a summation over the ensemble of noise vectors
	...
	for(kk = 0 ; kk < num_momenta*geo.L[0] ; kk++)
	{
	  W12[i][j][kk] = (qcd_complex_16) {0,0};         // for each noise vector we have to set the values of them to zero
	  W34[i][j][kk] = (qcd_complex_16) {0,0};
	  W56[i][j][kk] = (qcd_complex_16) {0,0};
	  W78[i][j][kk] = (qcd_complex_16) {0,0};
	}
    ...
    for( it = x_src[0] + 1 ; it < t_noise ; it++)                                         
      {
	...
	for(i=0; i < Nmu ; i++)
	  for(j=0; j < Nic ; j++)
	    for(v=0; v<geo.lV3; v++)
	      W2[i][j][v] = (qcd_complex_16) {0,0};          // this term must be zero and for noise loop & for time loop
	...
	for(lx = 0 ; lx < geo.lL[1] ; lx++)                  // each processors do his work for its local lattice
	  for(ly = 0 ; ly< geo.lL[2] ; ly++)
	    for(lz = 0 ; lz < geo.lL[3] ; lz++){
	      ...
	      // for W2 term
	      for(tt=0 ; tt< Nic ;tt++)
		for( ctr2 =0 ; ctr2 < ctr11 ; ctr2++){
		  zita = gammag5_ind[ctr2][0];
		  phita = gammag5_ind[ctr2][1];
		  
		  for(hh=0; hh<Nic ;hh++)
		    for(sigma =0 ;sigma<Nmu ; sigma++)
		      W2[sigma][hh][v3] = qcd_CADD(W2[sigma][hh][v3], 
						   qcd_CMUL(gammag5_val[ctr2],qcd_CMUL(qcd_CONJ(phi.D[v][zita][tt]),uprop_unsmear.D[v][phita][sigma][tt][hh])));
	...
	for( j=0 ; j< num_momenta ; j++){ // begin fourier
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
		...
		for( hh = 0 ; hh < Nic ; hh++){
		  W12[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W12[delta][sigma][q*geo.L[0]+t],qcd_CMUL(W1p_r[delta][hh][0] , W2p_r[sigma][hh][q]));
		  W78[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W78[delta][sigma][q*geo.L[0]+t],qcd_CMUL(W7p_r[delta][hh][0] , W2p_r[sigma][hh][q]));
		  
		  for(int lambda = 0 ; lambda < Nmu ; lambda++){
		     W34[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W34[delta][sigma][q*geo.L[0]+t],qcd_CMUL(W3p_r[delta][sigma][lambda][hh][0],W2p_r[lambda][hh][q]));
		     W56[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W56[delta][sigma][q*geo.L[0]+t],qcd_CMUL(W5p_r[delta][sigma][lambda][hh][0],W2p_r[lambda][hh][q]));
		  }
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
	    for( delta = 0 ; delta < Nmu ; delta++)
	      for( sigma = 0 ; sigma < Nmu ; sigma++){
		threep[delta][sigma][q*geo.L[0]+t] = qcd_CADD(threep[delta][sigma][q*geo.L[0]+t],qcd_CADD(qcd_CSUB(W12[delta][sigma][q*geo.L[0]+t],
               W34[delta][sigma][q*geo.L[0]+t]),qcd_CSUB(W56[delta][sigma][q*geo.L[0]+t], W78[delta][sigma][q*geo.L[0]+t])));
		threep_test[delta][sigma][q*geo.L[0]+t] = threep[delta][sigma][q*geo.L[0]+t];
	      }
 ...
 if(myid == 0){
    int q=0;
      for(int it = x_src[0] + 1 ; it < t_noise ; it++)
	{
	  ...
	  for(int delta = 0 ; delta < Nmu ; delta++)
	    for(int sigma = 0 ; sigma < Nmu ; sigma++){
	      threep_test[delta][sigma][q*geo.L[0]+t].re = -(threep_test[delta][sigma][q*geo.L[0]+t].re/(r+1));
	      threep_test[delta][sigma][q*geo.L[0]+t].im = -(threep_test[delta][sigma][q*geo.L[0]+t].im/(r+1));
	    }
	  for(kk=0; kk < geo.L[0] ; kk++) threep_proj[kk] = (qcd_complex_16) {0,0};
	  for(int delta = 0 ; delta < Nmu ; delta++)
            for(int sigma = 0 ; sigma < Nmu ; sigma++){
	      threep_proj[q*geo.L[0]+t] = qcd_CADD(threep_proj[q*geo.L[0]+t],qcd_CMUL(PROJECTOR[proj_index][delta][sigma],threep_test[sigma][delta][q*geo.L[0]+t]));   
            }
	  
	  fprintf(fp_threep,"%+d %d %e %e\n",t,r+1,threep_proj[q*geo.L[0]+t].re,threep_proj[q*geo.L[0]+t].im);
	}  
  }
