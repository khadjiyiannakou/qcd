...
//   if(myid==0) printf("Got solution-list file name: %s\n",sollist_name);
   ...
   for(mu=0;mu<Nmu;mu++)
   for(nu=0;nu<Nmu;nu++){
    C.re = -qcd_G5GAMMA[gamma_index][mu][nu].re;
    C.im = -qcd_G5GAMMA[gamma_index][mu][nu].im;
    if(qcd_NORM(C) > 1e-3){
       ...
       for(int lx=0;lx<Phi.geo->lL[1];lx++){
        for(int ly=0;ly<Phi.geo->lL[2];ly++){
         for(int lz=0;lz<Phi.geo->lL[3];lz++){
           ...
           block[v3] =qcd_CADD(block[v3],qcd_CMUL(qcd_G5GAMMA[gamma_index][mu][nu],qcd_CMUL(qcd_CONJ(Phi.D[qcd_LEXIC(it,lx,ly,lz,geo.lL)][mu][ic]),Phi.D[qcd_LEXIC(it,lx,ly,lz,geo.lL)][nu][ic])));
