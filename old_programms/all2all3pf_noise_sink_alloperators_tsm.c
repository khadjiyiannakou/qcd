  ...
  for(int r = 0 ; r < num_phi ; r++){        // a summation over the ensemble of noise vectors
	...
	for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
	    for( delta = 0 ; delta < Nmu ; delta++)
	      for( sigma = 0 ; sigma < Nmu ; sigma++){
		threep[gamma_index][delta][sigma][q*Tseperation+it] = qcd_CADD(threep[gamma_index][delta][sigma][q*Tseperation+it],qcd_CADD(qcd_CSUB(W12[gamma_index][delta][sigma][q],
	      ...
	      for(int it = 0 ; it < Tseperation ; it++)
		{
		  ...
		  for(int delta = 0 ; delta < Nmu ; delta++)
		    for(int sigma = 0 ; sigma < Nmu ; sigma++){
		      threep_proj[gamma_index][q*Tseperation+it] = qcd_CADD(threep_proj[gamma_index][q*Tseperation+it],qcd_CMUL(PROJECTOR[proj_index][delta][sigma],threep[gamma_index][sigma][delta][q*Tseperation+it]));   
		    }
