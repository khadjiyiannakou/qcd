  ...
  */
  ...
  for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
    ...
    for( i = 0 ; i < Nmu ; i++)
      for(j = 0 ; j <Nmu ; j++)
        {
          W12[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
          W34[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
          W56[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
          W78[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
          threep[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
          threep_test[gamma_index][i][j] = (qcd_complex_16*)allocate(num_momenta*Tseperation*sizeof(qcd_complex_16));
  ...
  if(myid == 0)printf("finish all allocation\n");
  //  exit(-1);
