...
int main(int argc, char* argv[]){
  ...
  qcd_int_2  gammag5_ind[16*16][4], cg5g5cg5_ind[16*16][4]; // store the non zero indices and gamma values
  qcd_complex_16  gammag5_val[16*16], cg5g5cg5_val[16*16];  // %
  ...
  qcd_complex_16 *W1[Nmu][Nic], *W2[Nmu][Nmu][Nmu][Nic], *W3[Nmu][Nmu][Nmu][Nic]; //         W terms for contractions    
  qcd_complex_16 *W1p[Nmu][Nic], *W2p[Nmu][Nmu][Nmu][Nic], *W3p[Nmu][Nmu][Nmu][Nic]; //W8=W6=W4=W2    W terms after fourier
  qcd_complex_16 *W1p_r[Nmu][Nic], *W2p_r[Nmu][Nmu][Nmu][Nic], *W3p_r[Nmu][Nmu][Nmu][Nic];        // W after reduce to root
  qcd_complex_16 *W12[Nmu][Nmu], *W13[Nmu][Nmu];                                                    // after do all the color-spin sums
 ...
 //set up MPI                                                                                 
  ...
  sscanf(qcd_getParam("<proj_index>",params,params_len),"%d",&proj_index);             
  ...
  for( i=0; i < Nmu ; i++)
    for( j=0; j<Nmu ; j++)
      for(k=0; k<Nmu ; k++)
	for(l=0; l<Nic ; l++){
	  W2[i][j][k][l] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
	  W3[i][j][k][l] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
	  W2p[i][j][k][l] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
	  W3p[i][j][k][l] =(qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
	  W2p_r[i][j][k][l] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
  ...
  for( i = 0 ; i < Nmu ; i++)
	...
	W13[i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
  ...
  for(int r = 0 ; r < num_phi ; r++){        // a summation over the ensemble of noise vectors
	...
	for(i=0; i < Nmu ; i++)
	  for(j=0; j < Nic ; j++)
	    for(v=0; v<geo.lV3; v++)
	      W1[i][j][v] = (qcd_complex_16) {0,0};          // this term must be zero and for noise loop & for time loop
		...
		for( ctr2 =0 ; ctr2 < ctr11 ; ctr2++){
		  ...
		  for(gg=0; gg<Nic ;gg++)
		    for(xita =0 ;xita<Nmu ; xita++)
		      W1[xita][gg][v3] = qcd_CADD(W1[xita][gg][v3], 
						   qcd_CMUL(gammag5_val[ctr2],qcd_CMUL(qcd_CONJ(phi.D[v][zita][tt]),dprop.D[v][phita][xita][tt][gg])));
	...
	for( j=0 ; j< num_momenta ; j++){ // begin fourier
	  ...
	  for(lx=0; lx<geo.lL[1]; lx++)
	      ...
	      for(lz=0; lz<geo.lL[3]; lz++)
		{
		  ...
		  for( gg = 0 ; gg < Nic ; gg++)
		    for( xita = 0 ; xita < Nmu ; xita++){
		      W1p[xita][gg][j]=qcd_CADD(W1p[xita][gg][j], qcd_CMUL(W1[xita][gg][v3],C2));
	  ...
	  for( gg = 0 ; gg < Nic ; gg++) // reduce the values to root
	    for(xita = 0 ;xita <Nmu ; xita++){
	      MPI_Reduce(&(W1p[xita][gg][j].re), &(W1p_r[xita][gg][j].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	      for( delta = 0 ; delta < Nmu ; delta++)
		for(sigma = 0 ; sigma < Nmu ; sigma++){
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
		...
		for( gg = 0 ; gg < Nic ; gg++)
		  for(xita = 0 ; xita < Nmu ; xita++){
		    W12[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W12[delta][sigma][q*geo.L[0]+t],
						      qcd_CMUL(W2p_r[delta][sigma][xita][gg][0],W1p_r[xita][gg][q]));
		    W13[delta][sigma][q*geo.L[0]+t] = qcd_CADD(W13[delta][sigma][q*geo.L[0]+t],
						      qcd_CMUL(W3p_r[delta][sigma][xita][gg][0],W1p_r[xita][gg][q]));
		  }
	      }
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
	    for( delta = 0 ; delta < Nmu ; delta++)
	      for( sigma = 0 ; sigma < Nmu ; sigma++){
		threep[delta][sigma][q*geo.L[0]+t] = qcd_CADD(threep[delta][sigma][q*geo.L[0]+t],
							      qcd_CSUB(W12[delta][sigma][q*geo.L[0]+t],W13[delta][sigma][q*geo.L[0]+t]));
