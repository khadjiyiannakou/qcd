...
int main(int argc, char* argv[]){
 ...
 //set up MPI                                                                                 
  ...
  for(int gamma_index = 0; gamma_index < Noperators ; gamma_index++){
    for(i=0; i < Nmu ; i++)
      for(j=0; j < Nic ; j++){
	W1[gamma_index][i][j] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
	W1p[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
	W1p_r[gamma_index][i][j] =(qcd_complex_16*)malloc(num_momenta *sizeof(qcd_complex_16));
      }
  }
	...
	for(j = 0 ; j <Nmu ; j++)
	  {
	    ...
	    W13[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	...
	for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
	  for( int q = 0 ; q < num_momenta; q++)
		...
		for( gg = 0 ; gg < Nic ; gg++)
		  for(xita = 0 ; xita < Nmu ; xita++){
		    ...
		    W13[gamma_index][delta][sigma][q*geo.L[0]+t] = qcd_CADD(W13[gamma_index][delta][sigma][q*geo.L[0]+t],
	  ...
	  for( int q = 0 ; q < num_momenta; q++)
		...
		threep[gamma_index][delta][sigma][q*geo.L[0]+t] = qcd_CADD(threep[gamma_index][delta][sigma][q*geo.L[0]+t],
									   qcd_CSUB(W12[gamma_index][delta][sigma][q*geo.L[0]+t],W13[gamma_index][delta][sigma][q*geo.L[0]+t]));
	      }
	}
	...
	for(int it = 0 ; it < Tseperation ; it++)
	  {
		...
		threep[gamma_index][delta][sigma][q*geo.L[0]+t].re = (threep[gamma_index][delta][sigma][q*geo.L[0]+t].re/num_phi);
		threep[gamma_index][delta][sigma][q*geo.L[0]+t].im = (threep[gamma_index][delta][sigma][q*geo.L[0]+t].im/num_phi);
	      }

	    for(int delta = 0 ; delta < Nmu ; delta++)
	      for(int sigma = 0 ; sigma < Nmu ; sigma++){
		threep_proj[gamma_index][q*geo.L[0]+t] = qcd_CADD(threep_proj[gamma_index][q*geo.L[0]+t],qcd_CMUL(PROJECTOR[proj_index][delta][sigma],threep[gamma_index][sigma][delta][q*geo.L[0]+t]));
	      }

	 fprintf(fp_threep,"%d %d %d %e %e\n",gamma_index,it,q,threep_proj[gamma_index][q*geo.L[0]+t].re,threep_proj[gamma_index][q*geo.L[0]+t].im);
