...
#include "seq_fix_current_input/particles_part.h"          // this file includes the insertion parts for all particles
  ...
  qcd_complex_16 *part0[MAX_PARTICLES][NPROJECTORS], *part1[MAX_PARTICLES][NPROJECTORS], *part2[MAX_PARTICLES][NPROJECTORS]; // part for each insertion type
  ...
  //////////////////////////// read momenta ///////////////////////////////////
    ...
    for(int iparticle = 0 ; iparticle < MAX_PARTICLES ; iparticle++)
	  ...
	  part0[iparticle][iproj] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	  part1[iparticle][iproj] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	  part2[iparticle][iproj] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	}
    
    for(int iparticle = 0 ; iparticle < MAX_PARTICLES ; iparticle++)
      for(int iproj = 0 ; iproj <  NPROJECTORS ; iproj++)
	  for(int itmom =0 ; itmom < number_tsinks * num_momenta ; itmom++){
	    part0[iparticle][iproj][itmom].re = 0; part0[iparticle][iproj][itmom].im = 0;
	    part1[iparticle][iproj][itmom].re = 0; part1[iparticle][iproj][itmom].im = 0;
	    part2[iparticle][iproj][itmom].re = 0; part2[iparticle][iproj][itmom].im = 0;	  
	  }
   ...
   ////////// from 0-7 octet particles ///////////////////////////////////////
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[8] = 1 ;
	function_flag = tpf_delta_plus_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[9] = 1 ;
	function_flag = tpf_delta_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[10] = 1 ;
	function_flag = tpf_delta_zero_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[11] = 1 ;
	function_flag = tpf_delta_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[12] = 1 ;
	function_flag = tpf_sigma_star_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[13] = 1 ;
	function_flag = tpf_sigma_star_zero_J32_sp(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[14] = 1 ;
	function_flag = tpf_sigma_star_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[15] = 1 ;
	function_flag = tpf_ksi_star_zero_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[16] = 1 ;
	function_flag = tpf_ksi_star_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[17] = 1 ;
	function_flag = tpf_omega_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
	...
	particles_flag[23] = 1 ;
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_PLUS_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[24] = 1 ;
        function_flag = tpf_sigma_star_plus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[25] = 1 ;
        function_flag = tpf_sigma_star_zero_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[26] = 1 ;
        function_flag = tpf_sigma_star_minus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_STAR_PLUS_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[27] = 1 ;
        function_flag = tpf_ksi_star_zero_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_STAR_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[28] = 1 ;
        function_flag = tpf_ksi_star_minus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
      ...
      if( !(strcmp(particle_list[iparticle],"OMEGA_PLUS_PLUS_C_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[29] = 1 ;
        function_flag = tpf_omega_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[36] = 1 ;
	function_flag = tpf_ksi_star_zero_J32(&cprop, &dprop, &sprop, &seq_cprop, &seq_dprop,     // here the second propagator will not be used
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[37] = 1 ;
	function_flag = tpf_decuplet_uds_J32(&sprop, &dprop, &cprop, &seq_sprop, &seq_dprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[38] = 1 ;
	function_flag = tpf_decuplet_uds_J32(&sprop, &uprop, &cprop, &seq_sprop, &seq_uprop,
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[39] = 1 ;
	function_flag = tpf_ksi_star_zero_J32(&sprop, &dprop, &cprop, &seq_sprop, &seq_dprop,     // here the second propagator will not be used
