...
{
   qcd_int_2 mu,nu,ku,lu,c1,c2,id1,id2,id3;  // various loop variables
   qcd_int_4 i,j,k,v,lx,ly,lz,ip1,im1; 
   qcd_int_4 x,y,z;
   qcd_int_4 numOfMom;                       // number of momenta
   qcd_int_2 ic1,ic2,ic3;                    //
   qcd_int_4 x_src[4];                       // source and sink coordinates
   qcd_int_4 t_sink, t_start, t_stop, t,lt;
...
*/
   ...
   for(lt=0; lt<geo.lL[0]; lt++)
   {
     ...
     // phase_factor_b.re = cos( 3.14159265359*(3*0-t-2*0) / ((double) geo.L[0] ));
     // phase_factor_b.im = -sin(theta[0]*(3*t_sink-t-2*x_src[0]) / ((double) geo.L[0] ));
     /*
     phase_factor   = (qcd_complex_16) {cos(theta[0]*(t-x_src[0])/ ((double) geo.L[0])),sin(theta[0]*(t-x_src[0])/( (double) geo.L[0]))};
     phase_factor_b = (qcd_complex_16) {cos(theta[0]*(3*t_sink-t-2*x_src[0])/((double) geo.L[0] )),-sin(theta[0]*(3*t_sink-t-2*x_src[0])/((double) geo.L[0]) )};
      
    if(myid == 0)printf("%d %d %d %d %+e %+e\n",t_sink,t,x_src[0], geo.L[0], phase_factor_b.re,phase_factor_b.im);
      for(int iz = 0 ; iz < geo.lL[3] ; iz++)
	for(int iy = 0 ; iy < geo.lL[2] ; iy++)
	  for(int ix = 0 ; ix < geo.lL[1] ; ix++){

	    int pos = qcd_LEXIC(lt,ix,iy,iz,geo.lL);
	    for(int mu = 0 ; mu < 4 ; mu++)
	      for(int nu = 0 ; nu < 4 ; nu++)
		for(int c1 = 0 ; c1 < 3 ; c1++)
		  for(int c2 = 0 ; c2 < 3 ; c2++){
		    prop.D[pos][mu][nu][c1][c2] = qcd_CMUL(prop.D[pos][mu][nu][c1][c2],phase_factor);
		    backprop.D[pos][mu][nu][c1][c2] = qcd_CMUL(backprop.D[pos][mu][nu][c1][c2],phase_factor_b);
		  }
      }
     
     qcd_mulPropagatorC3d(&prop, phase_factor, t % geo.L[0]);
     qcd_mulPropagatorC3d(&backprop, phase_factor_b, t % geo.L[0]);
     */ 
   }
   //   if(myid == 0)printf("%+e %+e\n",backprop.D[1][0][0][0][0].re , backprop.D[1][0][0][0][0].im);
