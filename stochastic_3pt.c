...
int main(int argc, char* argv[]){
  ...
  char output_name_up[qcd_MAX_STRING_LENGTH];     // name of output file for up
  char output_name_down[qcd_MAX_STRING_LENGTH];

  char phi_name_up[qcd_MAX_STRING_LENGTH];        // file with the filenames of phi low precision solution vector
  char xi_name_up[qcd_MAX_STRING_LENGTH];      
  char phi_name_down[qcd_MAX_STRING_LENGTH];        // file with the filenames of phi low precision solution vector                                                                                
  char xi_name_down[qcd_MAX_STRING_LENGTH];      

  FILE *fp_phi_name_up,*fp_xi_name_up,*fp_phi_name_down,*fp_xi_name_down; // file pointer to the different input files

  char phi_files_up[MAX_NUM_PHI][qcd_MAX_STRING_LENGTH];        // for each number of phi solution vector we store a string for the path
  char xi_files_up[MAX_NUM_PHI][qcd_MAX_STRING_LENGTH];       
  char phi_files_down[MAX_NUM_PHI][qcd_MAX_STRING_LENGTH];        // for each number of phi solution vector we store a string for the path                                                          
  char xi_files_down[MAX_NUM_PHI][qcd_MAX_STRING_LENGTH];

  qcd_uint_4 num_phi, num_xi; // the num_phi == num_xi_1, num_chi == num_xi_2 check this must be equal 
  ...
  qcd_propagator uprop_unsmear, dprop_unsmear;
  ...
  qcd_vector phi_up,xi_up,phi_down,xi_down;                    // solution and noice vectors
  ...
  //for down
  qcd_int_4 ctr_down, ctr2_down, ctr11_down[Noperators], ctr22_down, ctr33_down;           // special spin indices trick for gammas multiplications                                                 
  qcd_int_2  gammag5_ind_down[Noperators][16][2], cg5g5cg5_ind[16*16][4]; // store the non zero indices and gamma values                                                   
  qcd_complex_16  gammag5_val_down[Noperators][16], cg5g5cg5_val[16*16];  // %                                                                                                   
  qcd_complex_16 gammag5_down[Noperators][4][4], g5cg5[4][4];
  ...
  // for down
  qcd_complex_16 *W1_down[Noperators][Nmu][Nic], *W2_down[Nmu][Nmu][Nmu][Nic], *W3_down[Nmu][Nmu][Nmu][Nic]; //         W terms for contractions     
  qcd_complex_16 *W1p_down[Noperators][Nmu][Nic], *W2p_down[Nmu][Nmu][Nmu][Nic], *W3p_down[Nmu][Nmu][Nmu][Nic]; //W8=W6=W4=W2    W terms after fourier     
  qcd_complex_16 *W1p_r_down[Noperators][Nmu][Nic], *W2p_r_down[Nmu][Nmu][Nmu][Nic], *W3p_r_down[Nmu][Nmu][Nmu][Nic];        // W after reduce to root                                              
  qcd_complex_16 *W12_down[Noperators][Nmu][Nmu], *W13_down[Noperators][Nmu][Nmu];                                  // after do all the color-spin sums                                             
  qcd_complex_16 *threep_down[Noperators][Nmu][Nmu], *threep_proj_down[Noperators], *threep_test_down[Noperators][Nmu][Nmu];                                                
 ...
 //set up MPI                                                                                 
  ...
  strcpy(output_name_up,qcd_getParam("<output_name_up>",params,params_len)); //get  output filename                                                                
  if(myid==0) printf("Got output file name: %s\n",output_name_up);

  strcpy(output_name_down,qcd_getParam("<output_name_down>",params,params_len)); //get  output filename                      
  if(myid==0) printf("Got output file name: %s\n",output_name_down);
  ...
  strcpy(phi_name_up,qcd_getParam("<phi_list_up>",params,params_len)); //get the filename of phi list                                                       
  ...
  strcpy(xi_name_up,qcd_getParam("<xi_list_up>",params,params_len)); //get the filename of xi_1 list                                   
  if(myid==0) printf("Got xi-list file name: %s\n",xi_name_up);

  sscanf(qcd_getParam("<num_xi>",params,params_len),"%d",&num_xi); // read number of xi_1 vectors                                                                  
  if(myid == 0)printf("Number of xi_1 vectors is: %d\n",num_xi);

  strcpy(phi_name_down,qcd_getParam("<phi_list_down>",params,params_len)); //get the filename of phi list       
  if(myid==0) printf("Got phi-list-down file name: %s\n",phi_name_down);

  strcpy(xi_name_down,qcd_getParam("<xi_list_down>",params,params_len)); //get the filename of xi_1 list
  if(myid==0) printf("Got xi-list file name: %s\n",xi_name_down);


  if( num_phi != num_xi )
    {
      if(myid == 0)fprintf(stderr,"Error, num_phi must be the same with num_xi");
      exit(EXIT_FAILURE);
    }
  ...
  j += qcd_initPropagator(&dprop_unsmear, &geo);  
  j += qcd_initVector(&vec, &geo);

  j += qcd_initVector(&phi_up, &geo);       // initialize phi_up vector
  j += qcd_initVector(&xi_up, &geo);      // initialize xi_up vector
  j += qcd_initVector(&phi_down, &geo);       // initialize phi_up vector 
  j += qcd_initVector(&xi_down, &geo);      // initialize xi_up vector       
  ...
  qcd_copyPropagatorPropagator(&dprop_unsmear, &dprop); // copy to one no smeared propagator before smearing                                                                                        
    ...
    for(xita = 0 ; xita <4 ; xita++)
      for(beta =0; beta < 4 ; beta++)
        for(kappa=0; kappa < 4 ; kappa++){
          ...
          if(qcd_NORM(C) > 1e-3)
            {
              cg5g5cg5_val_down[ctr33].re = C.re;
              cg5g5cg5_val_down[ctr33].im = C.im;
              cg5g5cg5_ind_down[ctr33][0] = lambda;
              cg5g5cg5_ind_down[ctr33][1] = xita;
              cg5g5cg5_ind_down[ctr33][2] = beta;
              cg5g5cg5_ind_down[ctr33][3] = kappa;
              ctr33_down++;
            }
        }
  ...
  for(int gamma_index = 0 ; gamma_index < Noperators ; gamma_index++){
    for(zita = 0 ; zita < 4 ; zita++)
      for(phita = 0 ; phita < 4 ; phita++){
        ...
        gammag5_down[gamma_index][zita][phita] = sum ;
      }
  }

  for(int gamma_index = 0 ; gamma_index < Noperators ; gamma_index++){
    ctr11_down[gamma_index] = 0 ;
    for(zita =0; zita < 4 ; zita++)                               // now for gamma inserion matrix only                                                                                              
      for(phita=0; phita < 4 ; phita++){
        C = gammag5_down[gamma_index][zita][phita] ;
        if(qcd_NORM(C) > 1e-3)
          {
            gammag5_val_down[gamma_index][ctr11[gamma_index]].re = C.re;
            gammag5_val_down[gamma_index][ctr11[gamma_index]].im = C.im;
            gammag5_ind_down[gamma_index][ctr11[gamma_index]][0] = zita;
            gammag5_ind_down[gamma_index][ctr11[gamma_index]][1] = phita;
            ctr11_down[gamma_index]++;
          }
      }
  }
  ...
  if(myid==0)
    {
      fp_threep_up = fopen(output_name_up,"w");
      if(fp_threep_up==NULL)
	{
	  printf("failed to open %s for writing\n",output_name_up);
	  k=1;
	}
      fp_threep_down = fopen(output_name_down,"w");
      if(fp_threep_down==NULL)
        {
          printf("failed to open %s for writing\n",output_name_down);
          k=1;
        }
  ...
  ////////////////////////////////// allocate memory for down

  for(int gamma_index = 0; gamma_index < Noperators ; gamma_index++){
    for(i=0; i < Nmu ; i++)
      for(j=0; j < Nic ; j++){
        W1_down[gamma_index][i][j] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
        W1p_down[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
        W1p_r_down[gamma_index][i][j] =(qcd_complex_16*)malloc(num_momenta *sizeof(qcd_complex_16));
      }
  }
  ...
  for( i=0; i < Nmu ; i++)
    for( j=0; j<Nmu ; j++)
      for(k=0; k<Nmu ; k++)
        for(l=0; l<Nic ; l++){
          W2_down[i][j][k][l] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
          W3_down[i][j][k][l] = (qcd_complex_16*)malloc(geo.lV3 * sizeof(qcd_complex_16));
          W2p_down[i][j][k][l] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
          W3p_down[i][j][k][l] =(qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
          W2p_r_down[i][j][k][l] = (qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
          W3p_r_down[i][j][k][l] =(qcd_complex_16*)malloc(num_momenta * sizeof(qcd_complex_16));
        }
  for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
    threep_proj_down[gamma_index] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
    for( i = 0 ; i < Nmu ; i++)
      for(j = 0 ; j <Nmu ; j++)
	{
	  W12_down[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	  W13_down[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	  threep_down[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	  threep_test_down[gamma_index][i][j] = (qcd_complex_16*)malloc(num_momenta*geo.L[0]*sizeof(qcd_complex_16));
	}
  }
  ...
  for(int gamma_index =0 ;gamma_index < Noperators ; gamma_index++){
    for(kk=0; kk < num_momenta*geo.L[0] ; kk++){
      threep_proj_down[gamma_index][kk] = (qcd_complex_16) {0,0};
      for( i = 0 ; i < Nmu ; i++)
        for(j =0 ; j < Nmu ; j++)
          {
            threep_down[gamma_index][i][j][kk] = (qcd_complex_16) {0,0}; // initialize to zero the three point function                                                                                   
            threep_test_down[gamma_index][i][j][kk] = (qcd_complex_16) {0,0}; // initialize to zero the three point function                                                                              
          }
    }
  }
  ...
  fp_phi_name_up = fopen(phi_name_up,"r");
  fp_xi_name_up = fopen(xi_name_up,"r");
  fp_phi_name_down = fopen(phi_name_down,"r");
  fp_xi_name_down = fopen(xi_name_down,"r");

  
  if(fp_phi_name_up == NULL){
    if(myid == 0)fprintf(stderr,"Error opening list with phi up vectors\n");
    exit(EXIT_FAILURE);
  }
  if(fp_xi_name_up == NULL){
    if(myid == 0)fprintf(stderr,"Error opening list with xi up vectors\n");
    exit(EXIT_FAILURE);
  }
  
  for(int i=0;i<num_phi;i++)
    {
      fscanf(fp_phi_name_up,"%s",&(phi_files_up[i][0]));
    }  
    
fclose(fp_phi_name_up);


  for(int i=0;i<num_xi;i++)
    {
      fscanf(fp_xi_name_up,"%s",&(xi_files_up[i][0]));
    }
  
  fclose(fp_xi_name_up);

  if(fp_phi_name_down == NULL){
    if(myid == 0)fprintf(stderr,"Error opening list with phi down vectors\n");
    exit(EXIT_FAILURE);
  }
  if(fp_xi_name_down == NULL){
    if(myid == 0)fprintf(stderr,"Error opening list with xi down vectors\n");
    exit(EXIT_FAILURE);
  }

  for(int i=0;i<num_phi;i++)
    {
      fscanf(fp_phi_name_down,"%s",&(phi_files_down[i][0]));
    }

  fclose(fp_phi_name_down);


  for(int i=0;i<num_xi;i++)
    {
      fscanf(fp_xi_name_down,"%s",&(xi_files_down[i][0]));
    }

  fclose(fp_xi_name_down);
  ...
  for(int r = 0 ; r < num_phi ; r++){        // a summation over the ensemble of noise vectors
    ...
    for(int gamma_index = 0 ;gamma_index < Noperators ; gamma_index++){
      for( i = 0 ; i < Nmu ; i++)
        for(j = 0 ; j <Nmu ; j++)
          for(kk = 0 ; kk < num_momenta*geo.L[0] ; kk++)
            {
              W12_down[gamma_index][i][j][kk] = (qcd_complex_16) {0,0};         // for each noise vector we have to set the values of them to zero                                                        
              W13_down[gamma_index][i][j][kk] = (qcd_complex_16) {0,0};
            }
    }


    //load the current phi and xi_1
    qcd_getVector(&(phi_files_up[r][0]),qcd_PROP_LIME,0,0,&phi_up);
    qcd_getVector(&(xi_files_up[r][0]),qcd_PROP_LIME,0,0,&xi_up);
    qcd_getVector(&(phi_files_down[r][0]),qcd_PROP_LIME,0,0,&phi_down);
    qcd_getVector(&(xi_files_down[r][0]),qcd_PROP_LIME,0,0,&xi_down);
	...
	phase_factor   = (qcd_complex_16) {cos(theta[0]*t/geo.L[0]),sin(theta[0]*t/geo.L[0])};
	qcd_mulVectorC3d(&phi_up, phase_factor, (t+t_noise) % geo.L[0]);
	qcd_mulVectorC3d(&phi_down, phase_factor, (t+t_noise) % geo.L[0]);
      }
    ...
    for(i=0; i<nsmear; i++)
      {
    	if(qcd_gaussIteration3d(&xi_up,&uAPE,alpha,t_noise))
    	  {
    	    fprintf(stderr,"process %i: Error while smearing!\n",geo.myid);
    	    exit(EXIT_FAILURE);
    	  }
	if(qcd_gaussIteration3d(&xi_down,&uAPE,alpha,t_noise))
          {
            fprintf(stderr,"process %i: Error while smearing!\n",geo.myid);
            exit(EXIT_FAILURE);
          }
	...
	for( int q = 0 ; q < num_momenta; q++)
	  for(int it = 0 ; it < Tseperation ; it++)
		...
		for(int sigma = 0 ; sigma < Nmu ; sigma++){
	      ...
	      for(int delta = 0 ; delta < Nmu ; delta++)
		for(int sigma = 0 ; sigma < Nmu ; sigma++){
		  threep_proj[gamma_index][q*geo.L[0]+t] = qcd_CADD(threep_proj[gamma_index][q*geo.L[0]+t],qcd_CMUL(PROJECTOR[proj_index][delta][sigma\
																		 ],threep_test[gamma_index][sigma][delta][q*geo.L[0]+t]));
		}

	      fprintf(fp_threep,"%d %d %d %d %e %e\n",r,gamma_index,it,q,threep_proj[gamma_index][q*geo.L[0]+t].re,threep_proj[gamma_index][q*geo.L[0]+t].im);
