...
int main(int argc, char* argv[]){
  ...
  qcd_int_4 (*mom)[3];                         // momenta-list
   ...
   qcd_complex_16 corr_J32[4][4], Term3[4][4], Term6[4][4];
  ...
  free(params);
      ...
      if(myid==0)
      {
	for(int iparticle =8 ; iparticle < 12 ; iparticle++){
  ...
  ///////////////////////////////////////////////////////////////////////////////////
    ...
    ///////////////////////////////////////////// start contractions /////////////////////////////////////////////////////////////
	  ...
	  //	  octet_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);
	  // if(myid == 0) printf("finish octet\n");
	  
	  deltas_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);	     
	  if(myid == 0)printf("finish decuplet\n");
	 
	  //  octet_c_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);
	  // if(myid == 0) printf("finish octet_c\n");

	  // decuplet_c_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);            
	  //if(myid == 0)printf("finish decuplet_c\n");

	  // octet_final6_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);
          //if(myid == 0) printf("finish octet_final6\n");

	  //decuplet_final4_J32(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , cg5cg5b_ind , cg5cg5b_val);
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		...
		for(int mu = 0 ; mu < 4 ; mu++)
		  for(int nu = 0 ; nu < 4 ; nu++)
		    for(int lu = 0 ; lu < 4 ; lu++){
		      TermJ32[0][mu][nu] = qcd_CADD(TermJ32[0][mu][nu] , qcd_CMUL( qcd_GAMMA_J32[0][mu][lu] , qcd_CSUB(corr[iparticle][lu*4+nu][1*3+0] , corr[iparticle][lu*4+nu][0*3+1] ) ) );
		      TermJ32[1][mu][nu] = qcd_CADD(TermJ32[1][mu][nu] , qcd_CMUL( qcd_GAMMA_J32[1][mu][lu] , qcd_CSUB(corr[iparticle][lu*4+nu][2*3+0] , corr[iparticle][lu*4+nu][0*3+2] ) ) );
		      TermJ32[2][mu][nu] = qcd_CADD(TermJ32[2][mu][nu] , qcd_CMUL( qcd_GAMMA_J32[2][mu][lu] , qcd_CSUB(corr[iparticle][lu*4+nu][2*3+1] , corr[iparticle][lu*4+nu][1*3+2] ) ) );
		    }
		...
		for(int mu = 0 ; mu < 4 ; mu++)
		  for(int nu = 0 ; nu < 4 ; nu++){
		    ...
		    corr_J32[mu][nu] = qcd_CADD( qcd_CSUB( Term3[mu][nu] , Term6[mu][nu] ) , corr[iparticle][mu*4+nu][9] ) ;
		...
		for(int mu = 0 ; mu < 4 ; mu++)
		  fprintf(fp_twop[iparticle], "%d %d %+e %+e %+e %+e %+e %+e %+e %+e \n", it+after_tcur, imom, corr_J32[mu][0].re, corr_J32[mu][0].im,corr_J32[mu][1].re, corr_J32[mu][1].im,corr_J32[mu][2].re, corr_J32[mu][2].im,corr_J32[mu][3].re, corr_J32[mu][3].im);
		    //		    corr_proj = qcd_CADD(corr_proj , qcd_CMUL( PROJECTOR[13][mu][nu] , corr_J32[nu][mu] ) );
