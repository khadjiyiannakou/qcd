...
int main(int argc, char* argv[]){
  ...
  qcd_int_4 (*mom)[3];                         // momenta-list
   ...
   qcd_real_8 plaq;
   qcd_uint_4 nz_counter[4];
   qcd_int_2 Pcg5cg5b_ind[4][16*16*16][6];
   qcd_complex_16 Pcg5cg5b_val[4][16*16*16];
   qcd_complex_16 expon;
   
   int dummy;
   qcd_uint_4 num_momenta;
   qcd_complex_16 corr_temp[MAX_PARTICLES_2], corr[MAX_PARTICLES_2];
   
   qcd_complex_16 *block[MAX_PARTICLES_2];

   
   char *particles[MAX_PARTICLES_2] = {"PROTON", "NEUTRON", "LAMBDA", "SIGMA_PLUS", "SIGMA_ZERO", "SIGMA_MINUS", "KSI_ZERO", "KSI_MINUS", "DELTA_PLUS_PLUS", "DELTA_PLUS", "DELTA_ZERO", "DELTA_MINUS", 
     "SIGMA_STAR_PLUS", "SIGMA_STAR_ZERO", "SIGMA_STAR_MINUS", "KSI_STAR_ZERO", "KSI_STAR_MINUS", "OMEGA", "LAMBDA_C", "SIGMA_PLUS_C", "SIGMA_ZERO_C", "SIGMA_MINUS_C", "KSI_ZERO_C", "KSI_MINUS_C",
     "SIGMA_STAR_PLUS_C", "SIGMA_STAR_ZERO_C", "SIGMA_STAR_MINUS_C", "KSI_STAR_ZERO_C", "KSI_STAR_MINUS_C", "OMEGA_C"};
  ...
  ///////////////////////////////////////////////////////////////////////////////////
    ...
    malloc_flag = 0;
    for( int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++){
      block[iparticle] = (qcd_complex_16*) malloc(geo.lV3*sizeof(qcd_complex_16));
      if(block[iparticle] == NULL ) malloc_flag ++;
    }
    ...
    //////////////////// find non zero elements for Proj[gamma1][gamma] * Cg5 [alpha][beta] * Cg5b[beta1][alpha1] //////////////////
    
    for(int i=0; i < 4 ; i++) nz_counter[i] = 0;
	...
	for(int gamma = 0 ; gamma < 4 ; gamma++)
	  for(int alpha = 0 ; alpha < 4 ; alpha++)
		...
		for(int alpha1 = 0 ; alpha1 < 4 ; alpha1++){
		  C = qcd_CMUL(PROJECTOR[13][gamma1][gamma],qcd_CMUL(qcd_CGAMMA[list_cgamma[icg]][alpha][beta],qcd_BAR_CGAMMA[list_cgamma[icg]][beta1][alpha1]));
		  
		  if(qcd_NORM(C) > 1e-3){
		  Pcg5cg5b_val[icg][nz_counter[icg]].re = C.re;  
		  Pcg5cg5b_val[icg][nz_counter[icg]].im = C.im;
		  Pcg5cg5b_ind[icg][nz_counter[icg]][0] = gamma1;
		  Pcg5cg5b_ind[icg][nz_counter[icg]][1] = gamma;
		  Pcg5cg5b_ind[icg][nz_counter[icg]][2] = alpha;
		  Pcg5cg5b_ind[icg][nz_counter[icg]][3] = beta;
 		  Pcg5cg5b_ind[icg][nz_counter[icg]][4] = beta1;
 		  Pcg5cg5b_ind[icg][nz_counter[icg]][5] = alpha1;
		  nz_counter[icg]++;  
		  }
    ...
    ///////////////////////////////////////////// start contractions /////////////////////////////////////////////////////////////
	 ...
	 for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
	   for(int v3 =0 ; v3 < geo.lV3 ; v3++)
	     block[iparticle][v3] = (qcd_complex_16) {0,0};
	   
             
      octet(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , Pcg5cg5b_ind , Pcg5cg5b_val);
	if(myid == 0) printf("finish octet\n");

      decuplet(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , Pcg5cg5b_ind , Pcg5cg5b_val);	     
	if(myid == 0)printf("finish decuplet\n");
	 
      octet_c(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , Pcg5cg5b_ind , Pcg5cg5b_val);
        if(myid == 0) printf("finish octet_c\n");

      decuplet_c(&uprop, &dprop, &sprop, &cprop, &geo, block, t, nz_counter , Pcg5cg5b_ind , Pcg5cg5b_val);            
	 ...
	 //////////////////////////////// start fourier /////////////////////////////////////////////
	  ...
	  for(int imom = 0; imom < num_momenta; imom++){
	   if(myid == 0)printf("momentum %d\n",imom);
	    for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
		 corr_temp[iparticle] = (qcd_complex_16) {0,0} ;
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		 ...
		 for(int iparticle =0 ;iparticle < MAX_PARTICLES_2 ; iparticle++){
		   corr_temp[iparticle] = qcd_CADD(corr_temp[iparticle],qcd_CMUL(block[iparticle][v3],expon));
		 } // close spin indices
		 
            } // close space sum
            
            for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
		MPI_Reduce(&(corr_temp[iparticle].re),&(corr[iparticle].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

	    if(myid == 0){
	      for(int iparticle = 0 ; iparticle < MAX_PARTICLES_2 ; iparticle++)
	      fprintf(fp_twop[iparticle], "%d %d %+e %+e \n",it+after_tcur, imom, corr[iparticle].re , corr[iparticle].im); 
	    }
