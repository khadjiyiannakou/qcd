...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		 ...
		 for(int gamma1 =0 ;gamma1 < 4 ; gamma1++){
	      ...
	      itmom = it * num_momenta + imom ;
            for(int gamma =0 ; gamma < 4 ; gamma++)
	      for(int gamma1 =0 ;gamma1 < 4 ; gamma1++){
		MPI_Allreduce(&(tmp_corr[1][gamma][gamma1].re), &(D_part_ksiSM[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
		MPI_Allreduce(&(tmp_corr[2][gamma][gamma1].re), &(S_part_ksiSM[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
		// no Upart here
	      } // close MPI_Reduce
