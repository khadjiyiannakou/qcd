	  ...
	  for(int inz =0 ; inz < nz_counter ; inz++){
	    ...
	    for(int cc1=0;cc1<6;cc1++)
			...
			block_Dpart[gamma][gamma1][v3] = qcd_CSUB( block_Dpart[gamma][gamma1][v3] ,qcd_CMUL(qcd_CMUL(qcd_CMUL(qcd_CSCALE( 
			                                 ...
			                                 ,uprop->D[v][gamma][alpha1][c][a1] ), seq_dprop->D[v][beta][beta1][b][b1] ));
...
//               corr=qcd_CADD(corr, qcd_CMUL(block[v3],C2));
		 ...
		 for(int gamma1 =0 ;gamma1 < 4 ; gamma1++){
	      ...
	      itmom = it * num_momenta + imom ;
            for(int gamma =0 ; gamma < 4 ; gamma++)
	      for(int gamma1 =0 ;gamma1 < 4 ; gamma1++){
		MPI_Allreduce(&(tmp_corr[0][gamma][gamma1].re), &(U_part_proton[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
		MPI_Allreduce(&(tmp_corr[1][gamma][gamma1].re), &(D_part_proton[gamma][gamma1][itmom].re), 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
		// no Spart here
	      } // close MPI_Reduce
