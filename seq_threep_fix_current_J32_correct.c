...
#include <mpi.h>
#include <qcd.h>
#include <qcd_threep_seq_all_J32.h>
...
#include "seq_fix_current_input/particles_part.h"          // this file includes the insertion parts for all particles
  ...
  qcd_complex_16 *part0[MAX_PARTICLES][5], *part1[MAX_PARTICLES][5], *part2[MAX_PARTICLES][5]; // part for each insertion type
  int list_projectors[NPROJECTORS];       // list to store projectors that i will use
  ...
  //////////////////////////// read momenta ///////////////////////////////////
	...
	for(int i = 0 ; i < 5 ; i++){
	  part0[iparticle][i] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	  part1[iparticle][i] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	  part2[iparticle][i] = (qcd_complex_16*)allocate(number_tsinks*num_momenta*sizeof(qcd_complex_16));
	}
    
    for(int iparticle = 0 ; iparticle < MAX_PARTICLES ; iparticle++)
      for(int i = 0 ; i < 5 ; i++)
	for(int itmom =0 ; itmom < number_tsinks * num_momenta ; itmom++){
	  part0[iparticle][i][itmom].re = 0; part0[iparticle][i][itmom].im = 0;
	  part1[iparticle][i][itmom].re = 0; part1[iparticle][i][itmom].im = 0;
	  part2[iparticle][i][itmom].re = 0; part2[iparticle][i][itmom].im = 0;	  
	}
   ...
   ////////// from 0-7 octet particles ///////////////////////////////////////
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[0] = 1 ;
	function_flag = threep_proton(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[0], 
				   part1[0], part2[0], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[1] = 1 ;
	function_flag = threep_neutron(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[1], 
				   part1[1], part2[1], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[2] = 1 ;
	function_flag = threep_lambda(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[2], 
				   part1[2], part2[2], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[3] = 1 ;
	function_flag = threep_sigma_plus(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[3], 
				   part1[3], part2[3], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[4] = 1 ;
	function_flag = threep_sigma_zero(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[4], 
				   part1[4], part2[4], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[5] = 1 ;
	function_flag = threep_sigma_minus(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[5], 
				   part1[5], part2[5], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[6] = 1 ;
	function_flag = threep_ksi_zero(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[6], 
				   part1[6], part2[6], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[7] = 1 ;
	function_flag = threep_ksi_minus(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[7], 
				   part1[7], part2[7], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[8] = 1 ;
	function_flag = threep_delta_plus_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[8], 
				   part1[8], part2[8], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[9] = 1 ;
	function_flag = threep_delta_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[9], 
				   part1[9], part2[9], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[10] = 1 ;
	function_flag = threep_delta_zero_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[10], 
				   part1[10], part2[10], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[11] = 1 ;
	function_flag = threep_delta_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[11], 
				   part1[11], part2[11], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[12] = 1 ;
	function_flag = threep_sigma_star_plus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[12], 
				   part1[12], part2[12], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[13] = 1 ;
	function_flag = threep_sigma_star_zero_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[13], 
				   part1[13], part2[13], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[14] = 1 ;
	function_flag = threep_sigma_star_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[14], 
				   part1[14], part2[14], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[15] = 1 ;
	function_flag = threep_ksi_star_zero_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[15], 
				   part1[15], part2[15], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[16] = 1 ;
	function_flag = threep_ksi_star_minus_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[16], 
				   part1[16], part2[16], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[17] = 1 ;
	function_flag = threep_omega_J32(&uprop, &dprop, &sprop, &seq_uprop, &seq_dprop,
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[17], 
				   part1[17], part2[17], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	if(myid == 0)printf("Timing for OMEGA is %e in minutes \n",(end_time-start_time)/60.);
      ...
      if( !(strcmp(particle_list[iparticle],"LAMBDA_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[18] = 1 ;
        function_flag = threep_lambda(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[18], 
                                   part1[18], part2[18], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_PLUS_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[19] = 1 ;
        function_flag = threep_sigma_plus(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[19], 
                                   part1[19], part2[19], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[20] = 1 ;
        function_flag = threep_sigma_zero(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[20], 
                                   part1[20], part2[20], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[21] = 1 ;
        function_flag = threep_sigma_minus(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[21], 
                                   part1[21], part2[21], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_PLUS_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[22] = 1 ;
        function_flag = threep_ksi_zero(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[22], 
                                   part1[22], part2[22], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[23] = 1 ;
        function_flag = threep_ksi_minus(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[23], 
                                   part1[23], part2[23], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_PLUS_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[24] = 1 ;
        function_flag = threep_sigma_star_plus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[24], 
                                   part1[24], part2[24], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[25] = 1 ;
        function_flag = threep_sigma_star_zero_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[25], 
                                   part1[25], part2[25], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"SIGMA_STAR_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[26] = 1 ;
        function_flag = threep_sigma_star_minus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[26], 
                                   part1[26], part2[26], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_STAR_PLUS_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[27] = 1 ;
        function_flag = threep_ksi_star_zero_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[27], 
                                   part1[27], part2[27], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_STAR_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[28] = 1 ;
        function_flag = threep_ksi_star_minus_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[28], 
                                   part1[28], part2[28], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"OMEGA_PLUS_PLUS_C_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[29] = 1 ;
        function_flag = threep_omega_J32(&uprop, &dprop, &cprop, &seq_uprop, &seq_dprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[29], 
                                   part1[29], part2[29], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"OMEGA_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[30] = 1 ;
        function_flag = threep_proton(&sprop, &cprop, &cprop, &seq_sprop, &seq_cprop,              // here the third propagator will not be used
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[30], 
                                   part1[30], part2[30], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_PRIME_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[31] = 1 ;
        function_flag = threep_sigma_zero(&dprop, &sprop, &cprop, &seq_dprop, &seq_sprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[31], 
                                   part1[31], part2[31], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_ZERO_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[32] = 1 ;
        function_flag = threep_octet_uds(&dprop, &sprop, &cprop, &seq_dprop, &seq_sprop, 
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[32], 
                                   part1[32], part2[32], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_PRIME_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[33] = 1 ;
        function_flag = threep_sigma_zero(&uprop, &sprop, &cprop, &seq_uprop, &seq_sprop,
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[33], 
                                   part1[33], part2[33], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"KSI_PLUS_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[34] = 1 ;
        function_flag = threep_octet_uds(&uprop, &sprop, &cprop, &seq_uprop, &seq_sprop, 
                                   &seq_cprop, &geo, cur_time, number_tsinks, part0[34], 
                                   part1[34], part2[34], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
      ...
      if( !(strcmp(particle_list[iparticle],"OMEGA_PLUS_C_C")) ){  // check if this is the right particle
        ...
        if(myid == 0) start_time = MPI_Wtime();
	particles_flag[35] = 1 ;
        function_flag = threep_proton(&cprop, &sprop, &sprop, &seq_cprop, &seq_sprop,              // here the third propagator will not be used
                                   &seq_sprop, &geo, cur_time, number_tsinks, part0[35], 
                                   part1[35], part2[35], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
        if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[36] = 1 ;
	function_flag = threep_ksi_star_zero_J32(&cprop, &dprop, &sprop, &seq_cprop, &seq_dprop,     // here the second propagator will not be used
				   &seq_sprop, &geo, cur_time, number_tsinks, part0[36], 
				   part1[36], part2[36], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[37] = 1 ;
	function_flag = threep_decuplet_uds_J32(&sprop, &dprop, &cprop, &seq_sprop, &seq_dprop,
				   &seq_cprop, &geo, cur_time, number_tsinks, part0[37], 
				   part1[37], part2[37], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[38] = 1 ;
	function_flag = threep_decuplet_uds_J32(&sprop, &uprop, &cprop, &seq_sprop, &seq_uprop,
				   &seq_cprop, &geo, cur_time, number_tsinks, part0[38], 
				   part1[38], part2[38], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
	...
	if(myid == 0) start_time = MPI_Wtime();
	particles_flag[39] = 1 ;
	function_flag = threep_ksi_star_zero_J32(&sprop, &dprop, &cprop, &seq_sprop, &seq_dprop,     // here the second propagator will not be used
				   &seq_cprop, &geo, cur_time, number_tsinks, part0[39], 
				   part1[39], part2[39], &(mom[0]), num_momenta, x_src, after_tcur,list_projectors);
	if(myid == 0) end_time = MPI_Wtime();
  ...
  ////////////////////////////////open files for writting & act with projector and write 3pf //////////////////////
	...
	for(int iproj =0 ; iproj < NPROJECTORS ; iproj++)
	  for(int imom =0 ; imom < num_momenta ; imom++)
		...
		for(int ipart =0 ; ipart < 3 ; ipart++)		
		  if( (particles_parts[myid][ipart] != "z") && (particles_parts[myid][ipart] != "Z") ){
		    if(ipart == 0)fprintf(fp_threep[myid][ipart][iproj], "%d %d %+e %+e \n", imom, it + after_tcur, part0[myid][iproj][itmom].re, part0[myid][iproj][itmom].im);
		    if(ipart == 1)fprintf(fp_threep[myid][ipart][iproj], "%d %d %+e %+e \n", imom, it + after_tcur, part1[myid][iproj][itmom].re, part1[myid][iproj][itmom].im);
		    if(ipart == 2)fprintf(fp_threep[myid][ipart][iproj], "%d %d %+e %+e \n", imom, it + after_tcur, part2[myid][iproj][itmom].re, part2[myid][iproj][itmom].im);
		  }
